create or replace type body any_data_family_compound as
   ...
   member procedure set_data_values(self in out nocopy any_data_family_compound, p_data_values any_data_tab) is
      ...
      begin
         ...
         for i in 1 ..  c_cardinality loop
            if data_values(i) is not null then dbms_lob.writeappend( l_name_hashes,  16, data_values(i).get_name_hash() ); end if;
            if data_values(i) is not null then dbms_lob.writeappend( l_type_hashes,  16, data_values(i).get_type_hash() ); end if;
            if data_values(i) is not null then dbms_lob.writeappend( l_value_hashes, 16, data_values(i).get_value_hash() ); end if;
         end loop;
   ...
   begin
     return nvl(self.name_hash, any_data_const.null_hash_value);
   end;

   overriding member function get_value_hash return raw is
   begin
     return nvl(self.value_hash, any_data_const.null_hash_value);
   end;

   overriding member function get_type_hash return raw is
   begin
     return nvl(self.type_hash, any_data_const.null_hash_value);
   end;
